...
drupal8ci_install() {
	...
	set -ex
	# Download and extract Travis CI configuration and sample tests.
	wget -O "$tmpdir/master.zip" https://github.com/lcatlett/drupal8ci/archive/master.zip
	unzip "$tmpdir/master.zip" 'drupal8ci-master/dist/travisci/*' -d "$tmpdir"
	rsync -va --ignore-existing "$tmpdir/drupal8ci-master/dist/travisci/" .
